...
def prepare_agent_input(observation, prev_action, state):
    # SEED RL agent accepts input in a form of EnvOutput. When not training only observation is used for generating action, so we use a dummy values for the rest.
    ...
    # Add batch dimension.
    prev_action, env_output = tf.nest.map_structure(lambda t: tf.expand_dims(t, 0), (prev_action, env_output))
